...
app = Flask(__name__)
cors = CORS(app)
app.config['SQLALCHEMY_DATABASE_URI'] = "mysql+mysqlconnector://root:YES@localhost/Online_Shopping_project"
...
def get_customer_per_id(customer_id):
    ...
    if customer:
        ...
        return jsonify({"message": "Customer could not be found."}), 404
...
def update_customer_account(customer_account_id):
    try:
        ...
        if not customer_account: 
            return jsonify({"message": "Could not be found with that ID"}), 404
...
def delete_customer_account(customer_account_id):
    customer_account = CustomerAccount.query.filter(CustomerAccount.account_id == customer_account_id).first()
    if not customer_account: 
        return jsonify({"message": "Could not find customer account with ID"}), 404
...
@app.route("/products/restock_products", methods=["POST"])
